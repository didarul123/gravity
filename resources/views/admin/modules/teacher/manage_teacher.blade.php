...
<div class="container-fluid  dashboard-content">
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="page-header">
                <h2 class="pageheader-title">Manage Teacher</h2>
    ...
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            ...
            <div class="card">
                <h5 class="card-header">Manage Teacher <a class="adbtn btn btn-primary" title="Add new category" href="{{route('create.teacher')}}"><i class="fa fa-plus" aria-hidden="true"></i> Add Teacher</a></h5>
                <div class="card-body">
                    <form action="{{route('manage.teacher')}}">
                        <div class="row">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($val->status=='A')
                                        <a  href="{{route('change.teacher.status',[$val->id])}}" onclick="return confirm('Do you want to change status for this teacher ?')"><i class=" fas fa-times" title="Inactive"></i></a>
                                        @elseif($val->status=='I')
                                        <a href="{{route('change.teacher.status',[$val->id])}}" onclick="return confirm('Do you want to change status for this teacher ?')"><i class="fas fa-check" title="Active"></i></a>
                                        @endif
                                        <a href="{{route('view.teacher',[$val->id])}}"><i class="fas fa-eye" title="View"></i></a>
                                        <a href="{{route('create.teacher',[$val->id])}}"><i class=" fas fa-edit" title="Edit"></i></a>
                                        <a href="{{route('delete.teacher',[$val->id])}}" onclick="return confirm('Do you want to delete teacher ?')"><i class=" fas fa-trash" title="Delete"></i></a>
                                        @if(@$val->is_verified == 'N' && @$val->user_status != 'U' && @$val->user_status != 'D')
                                            <a href="{{route('teacher.verify',@$val->id)}}"title="Teacher Verified" onclick="return confirm('Are sure you want to verified this teacher?')"> <i class="fa fa-thumbs-up" aria-hidden="true"></i></a>
                                        @endif

                                        @if(@$val->is_verified == 'Y' && @$val->user_status != 'U' && @$val->user_status != 'D')
                                            <a href="{{route('teacher.verify',@$val->id)}}"title="Teacher Unverified" onclick="return confirm('Are sure you want to unverified this teacher?')"> <i class="fa fa-thumbs-down" aria-hidden="true"></i></a>
                                        @endif
